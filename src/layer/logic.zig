...
pub fn LogicSequential(gate_count: usize) type {
    return struct {
        ...
        pub fn backwardPass(
            ...
            input_buffer: *const [gate_count][2]f32,
            ...
            for (0..gate_count) |i| {
                const a = input_buffer[i][0];
                const b = input_buffer[i][1];
...
// const ConvolutionPlyOptions = struct {
//     height: usize,
//     width: usize,
//     kernel_count: usize,
//     field_size: struct { height: usize, width: usize },
...
// fn ConvolutionPly(options: ConvolutionPlyOptions) type {
//     return struct {
//         const Self = @This();
...
//         const Kernel = aesia.Network(&.{
//             aesia.layer.Repeat(field_height * field_width, 16),
//             aesia.layer.ButterflyMap(4, 0),
//             aesia.layer.ButterflyMap(4, 1),
//             aesia.layer.ButterflyMap(4, 2),
//             aesia.layer.ButterflyMap(4, 3),
//             aesia.layer.LogicSequential(1 << 3),
//             aesia.layer.LogicSequential(1 << 2),
//             aesia.layer.LogicSequential(1 << 1),
//         });
...
//             .trainable = true,
//             .parameter_count = kernel_count * Kernel.parameter_count,
//             .parameter_alignment = 64,
//         };

//         parameters: [kernel_count][Kernel.parameter_count]f32,
//         receptions: [height_out][width_out][field_height * field_width]f32,

//         pub fn init(self: *Self, parameters: *[lut_parameter_count][lut_count]f32) void {
//             for (0..kernel_count) |i| self.kernels[i].init(parameters[i]);
//         }

//         pub fn takeParameters(self: *Self, parameters: *[lut_parameter_count][lut_count]f32) void {
//             for (0..kernel_count) |i| self.kernels[i].takeParameters(parameters[i]);
//         }
...
//                 }
//             }
//             for (0..kernel_count) |i|_{
//                 for (0..height_out) |row| {
//                     for (0..width_out) |col| {
//                         output[i][row][col] = self.kernels[i].eval(&self.receptions[row][col])[0];
//                     }
//                 }
//             }
//         }
...
//             self: *Self,
//             input: *const [height_in][width_in]f32,
//             output: *[height_out][width_out][kernel_count]f32,
//         ) void {
//             return self.eval(input, output);
//         }
...
//                 for (0..height_out) |row| {
//                     for (0..width_out) |col| {
//                         const kernel = &self.kernels[i];
//                         @memcpy(kernel.lastDeltaBuffer(), &activation_delta[i][row][col]);
//                     }
//                 }
//             }
    ...
    return struct {
        ...
        pub fn backwardPass(
            ...
            for (0..dim) |i| {
                ...
                const beta = self.beta[i];
                const delta = activation_delta[i];
                cost_gradient[i] += delta * (1 - 2 * x) * beta * (1 - beta);
                argument_delta[i] = delta * (1 - 2 * beta);
            }
        }

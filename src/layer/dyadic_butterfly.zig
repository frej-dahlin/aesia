    ...
    return struct {
        ...
        pub fn eval(self: *const Self, input: *const [dim]f32, output: *[dim]f32) void {
            ...
            for (0..dim >> (stage + 1)) |k| {
                ...
                for (from..to) |j| {
                    ...
                    output[j] = @round((1 - c) * a + c * b);
                    output[j + delta] = @round(c * a + (1 - c) * b);
                }
            }
        }
        ...
        pub fn backwardPass(
            ...
            for (0..dim >> (stage + 1)) |k| {
                ...
                const to = (2 * k + 1) * delta;
                for (from..to) |j| {
                    const a = input_buffer[j];
                    const b = input_buffer[j + delta];
                    const c = self.steer[j];
        ...
        pub fn init(_: *Self, parameters: *[parameter_count]f32) void {
            for (0..dim >> (stage + 1)) |k| {
                ...
                for (from..to) |j| {
                    ...
                    parameters[index_left] = 2;
                    parameters[index_right] = -2;
                }
            }
        }
        ...
        pub fn eval(self: *const Self, input: *const [dim]f32, output: *[dim]f32) void {
            ...
            for (0..dim >> (stage + 1)) |k| {
                ...
                for (from..to) |j| {
                    ...
                    // gradients. By rounding the steering bits we converge faster towards a discrete
                    // network. Note that the derivative is still with respect to the unrounded
                    // value, otherwise the gradient would vanish.
        ...
        pub fn backwardPass(
            ...
            for (0..dim >> (stage + 1)) |k| {
                ...
                for (from..to) |j| {
                    const index_left = j;
                    const index_right = j + delta;
                    const a = input_buffer[index_left];
                    const b = input_buffer[index_right];
...
pub fn ButterflyGate(log2_dim: usize, stage: usize) type {
    ...
    return struct {
        ...
        pub fn init(_: *Self, parameters: *[dim]f32x4) void {
            for (0..dim >> (stage + 1)) |k| {
                ...
                for (from..to) |j| {
                    ...
                    // We initialize each gate switch towards a swap.
                    // This motivates the network to be influenced by all inputs.
                    parameters[index_left] = .{ 2, 2, -2, -2 };
                    parameters[index_right] = .{ 2, -2, 2, -2 };
                }
            }
        }
        ...
        pub fn eval(
            ...
            noalias input: *const [dim]f32,
            noalias output: *[dim]f32,
        ) void {
            @setFloatMode(.optimized);
            for (0..dim >> (stage + 1)) |k| {
                const from = 2 * k * delta;
                ...
                for (from..to) |j| {
                    ...
                    const a = input[index_left];
                    const b = input[index_right];
                    // Deep networks with many layers will eventually dilute the input
                    // values. The output values get pushed towards 0.5 which leads to vanishing
                    // gradients. By rounding the steering bits we converge faster towards a discrete
                    // network. Note that the derivative is still with respect to the unrounded
                    // value, otherwise the gradient would vanish.
        ...
        pub fn backwardPass(
            self: *const Self,
            noalias activation_delta: *const [dim]f32,
            noalias input_buffer: *const [dim]f32,
            noalias cost_gradient: *[dim]f32x4,
            noalias argument_delta: *[dim]f32,
        ) void {
            @setFloatMode(.optimized);
            @memset(argument_delta, 0);
            for (0..dim >> (stage + 1)) |k| {

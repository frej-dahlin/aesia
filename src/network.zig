...
pub fn LayeredCache(Layers: []const type) type {
    var list: []const type = &.{};
    for (Layers[1..Layers.len]) |Layer| list = list ++ .{Layer.info.Input()};
...
pub fn Network(Layers: []const type) type {
    ...
    inline for (Layers[0 .. Layers.len - 1], Layers[1..]) |prev, next| {
        ...
        if (prev.info.dim_out != next.info.dim_in) printCompileError(
            "Aesia: mismatched input/output dimension, {d} and {d} in layers:\n{s}\n{s}",
            .{
                prev.info.dim_out,
                next.info.dim_in,
                @typeName(prev),
    ...
    return struct {
        ...
        pub const info: aesia.layer.Info = .{
            .dim_in = dim_in,
            .dim_out = dim_out,
            // Todo: handle in_place and untrainable networks.
            .trainable = true,
        ...
        pub const ForwardCache = LayeredCache(Layers);
        ...
        pub fn eval(
            ...
            inline for (Layers, &self.layers, 0..) |Layer, *layer, l| {
                const buffer_in = if (l == 0) input else buffer.front(Layer.info.Input());
                const buffer_out = if (l == Layers.len - 1) output else buffer.back(Layer.info.Output());
                if (Layer.info.statefull) {
                    layer.eval(@ptrCast(buffer_in), @ptrCast(buffer_out));
                } else {
                    Layer.eval(@ptrCast(buffer_in), @ptrCast(buffer_out));
                }
                buffer.flip();
        ...
        pub fn validationEval(self: *Self, input: *const Input, output: *Output) void {
            @setEvalBranchQuota(1000 * Layers.len);
            const buffer = &self.buffer;
            inline for (Layers, &self.layers, 0..) |Layer, *layer, l| {
                const input_buffer = if (l == 0) input else buffer.front(Layer.info.Input());
                const output_buffer: *Layer.info.Output() = blk: {
                    if (l == Layers.len - 1) {
                        break :blk output;
                    } else if (Layer.info.in_place) {
                        break :blk @constCast(buffer.front(Layer.info.Output()));
                    } else {
                        break :blk buffer.back(Layer.info.Output());
                    }
                };
                ...
                if (l != Layers.len - 1 and !Layer.info.in_place) buffer.flip();
            }
        }
        ...
        pub fn forwardPass(
            ...
            const buffer = &self.buffer;
            inline for (Layers, &self.layers, 0..) |Layer, *layer, l| {
                const input_buffer = if (l == 0) input else &cache[l - 1];
                const output_buffer = if (l == Layers.len - 1) output else &cache[l];
                if (Layer.info.statefull) {
                    layer.forwardPass(@ptrCast(input_buffer), @ptrCast(output_buffer));
                } else {
                    Layer.forwardPass(@ptrCast(input_buffer), @ptrCast(output_buffer));
                }
                if (comptime l != Layers.len - 1 and !Layer.info.in_place) buffer.flip();
            }
        }

        pub fn backwardPass(
            self: *Self,
            last_activation_delta: *const [info.dim_out]f32,
            forward_input: *const [info.dim_in]f32,
            cache: *ForwardCache,
            gradient: *[parameter_count]f32,
            first_argument_delta: *[info.dim_in]f32,
        ) void {
            const buffer = &self.buffer;
            comptime var l: usize = Layers.len;
            @memset(first_argument_delta, 0);
            inline while (l > first_trainable_index) {
                l -= 1;
                const Layer = Layers[l];
                const layer = &self.layers[l];
                const activation_delta: *const [Layer.info.dim_out]f32 = if (l == Layers.len - 1)
                    last_activation_delta
                else
                    buffer.front([Layer.info.dim_out]f32);
                const argument_delta: *[Layer.info.dim_in]f32 = blk: {
                    if (l == 0) {
                        break :blk first_argument_delta;
                    } else {
                        break :blk buffer.back([Layer.info.dim_in]f32);
                    }
                };
                const forward_cache = if (l == 0) forward_input else &cache[l - 1];
                const range = parameter_ranges[l];
                // FIXME: Handle stateless layers.
                if (!Layer.info.trainable) {
                    layer.backwardPass(
                        @ptrCast(activation_delta),
                        @ptrCast(forward_cache),
                        @ptrCast(argument_delta),
                    );
                } else {
                    const gradient_slice = gradient[range.from..range.to()];
                    layer.backwardPass(
                        @alignCast(@ptrCast(activation_delta)),
                        @ptrCast(forward_cache),
        ...
        pub fn backwardPassFinal(
            ...
            inline while (l > first_trainable_index) {
                ...
                const Layer = Layers[l];
                const layer = &self.layers[l];
                const activation_delta = if (l == Layers.len - 1)
                    last_activation_delta
                else
                    buffer.front([Layer.info.dim_out]f32);
                const argument_delta: *[Layer.info.dim_in]f32 = if (Layer.info.in_place)
        ...
        pub fn outputBuffer(self: *Self) *[dim_out]f32 {
            const buffer = self.buffer.back([dim_out]f32);
            return buffer;
        }
    };
}
